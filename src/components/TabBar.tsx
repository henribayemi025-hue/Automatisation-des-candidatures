      ...
      <div className="pointer-events-auto flex items-center gap-1 rounded-pill bg-white/80 p-1.5 shadow-[0_10px_30px_rgba(23,27,38,0.18)] ring-1 ring-black/5 backdrop-blur-xl">
        {TABS.map((tab) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span style={{ color: isActive ? tab.color : '#6B6B6B' }}>
                  <tab.icon className="h-[22px] w-[22px]" />
                </span>
                {isActive && (
                  <span className="truncate text-[12px] font-semibold" style={{ color: tab.color }}>
                    {t(tab.label)}
                  </span>
                )}
              </>
        ...
        <button onClick={onMenu} aria-label={t('Menu')} className="flex flex-1 items-center justify-center rounded-pill py-2.5 text-muted active:scale-90">
