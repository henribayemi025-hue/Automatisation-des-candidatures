...
export default function Team() {
  ...
  async function invite(e: React.FormEvent) {
    ...
    if (err) setError(err);
      ...
      setOk(`${email} peut maintenant se connecter avec ce même email et rejoindre l’espace.`);
      setEmail('');
    }
  }
  ...
  if (!user) {
    return (
      <>
        <PageHeader title="Équipe" subtitle="Travailler à plusieurs sur le même espace" />
        <div className="card">
          <Empty
            title="Créez un compte pour inviter votre équipe"
            hint="En mode local, les données restent sur cet appareil. Avec un compte, votre caissier, votre gérant et votre comptable travaillent en direct sur les mêmes chiffres."
  ...
  return (
    <>
      <PageHeader title="Équipe" subtitle={`Espace « ${workspace?.name ?? ''} » — ${presence.length} personne(s) en ligne`} />

      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="card p-0">
          <h2 className="px-5 pb-3 pt-5 text-section">Membres</h2>
            ...
            <tr className="row">
              <td className="td">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold">{isOwner ? 'Vous' : 'Propriétaire'}</div>
            ...
            {members.map((m, i) => {
              ...
              return (
                <tr key={m.email} className="row">
                  ...
                  <td className="td">{ROLES.find((r) => r.value === m.role)?.label ?? m.role}</td>
                  <td className="td">
                    {online ? <Badge tone="success">En ligne</Badge> : m.userId ? <Badge>Hors ligne</Badge> : <Badge tone="warn">Pas encore connecté</Badge>}
                  </td>
          ...
          {members.length === 0 && (
            <p className="px-5 pb-5 text-caption text-muted">Vous êtes seul pour l’instant. Invitez quelqu’un à droite.</p>
          )}
        </div>

        <div className="card h-fit">
          <h2 className="text-section">Inviter quelqu’un</h2>
          <p className="mt-1 text-caption text-muted">
            La personne se connecte avec cet email (ou son compte Google) et retrouve l’espace immédiatement.
          </p>
          {!isOwner && <p className="mt-3 rounded-input bg-base px-3 py-2 text-caption text-muted">Seul le propriétaire peut inviter.</p>}
          ...
          <form onSubmit={invite} className="mt-4 space-y-4">
            <Field label="Email">
              <input id="invite-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} disabled={!isOwner} className="field" placeholder="caissier@exemple.com" />
            </Field>
            <Field label="Rôle">
                ...
                {ROLES.map((r) => (
                  <label key={r.value} className={`flex cursor-pointer items-start gap-3 rounded-input border p-3 ${role === r.value ? 'border-teal bg-teal-light' : 'border-hairline'}`}>
                    ...
                    <span>
                      ...
                      <span className="block text-caption text-muted">{r.hint}</span>
                    </span>
                  </label>
                ))}
              </div>
